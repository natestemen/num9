...
class Board:
    ...
    def choose_solid_base_high_top(
        ...
        *,
        ...
        Coefficients can be tuned to bias the heuristic:
            fill_weight: reward for total filled squares on the current layer
            hole_penalty: penalty for holes inside the bounding box on the layer
            support_weight: reward for the fraction of the piece that is supported
            edge_weight: reward for edge contacts on the same layer
            height_weight: reward for placing higher-valued tiles higher
            aspect_penalty_weight: penalty for rectangular (non-square) footprints
            low_height_penalty_weight: penalty for placing high-valued tiles on low layers
        """
        valid_moves = self.find_valid_moves(piece)

        original_rotation = piece.rotation
        best_moves: list[tuple[int, int]] = []  # (score, idx)
        for idx, (layer, i, j, r) in enumerate(valid_moves):
            ...
            layer_after = self.board[layer] + piece_on_board[layer]
            ...
            edges = self.num_edges_touching(piece, (layer, i, j))

            support_ratio = 1.0
            if layer > 0 and piece_tiles:
                support_below = self.board[layer - 1][
                    i : i + piece_height, j : j + piece_width
                ].copy()
                support_below[support_below > 0] = 1
                piece_mask = Board.blank_layer_with_piece(
                    piece, (0, 0), ones=True
            ...
            if footprint_indices[0].size:
                height_span = (
                    footprint_indices[0].max() - footprint_indices[0].min() + 1
                )
                width_span = (
                    footprint_indices[1].max() - footprint_indices[1].min() + 1
                )
                if height_span and width_span:
                    aspect_ratio = max(height_span, width_span) / min(
                        height_span, width_span
                    )
                    aspect_penalty = aspect_ratio - 1.0  # zero when square
        ...
        margin = abs(best_score) * 5e-2 + 1e-6  # treat nearly-equal scores as ties
